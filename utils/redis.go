...
func SetEX(key, value string, ex time.Duration) bool {
	result, err := RedisClient.Set(ctx, key, value, ex).Result()
	if err != nil {
		fmt.Println(err)
		return false
	}
...
func GetSet(key, value string) (bool, string) {
	oldValue, err := RedisClient.GetSet(ctx, key, value).Result()
	if err != nil {
		fmt.Println(err)
		return false, ""
	}
...
func IncrBy(key string, incr int64) int64 {
	val, err := RedisClient.IncrBy(ctx, key, incr).Result()
	if err != nil {
		fmt.Println(err)
	}
	return val
...
func IncrByFloat(key string, incrFloat float64) float64 {
	val, err := RedisClient.IncrByFloat(ctx, key, incrFloat).Result()
	if err != nil {
		fmt.Println(err)
	}
	return val
...
func DecrBy(key string, incr int64) int64 {
	val, err := RedisClient.DecrBy(ctx, key, incr).Result()
	if err != nil {
		fmt.Println(err)
	}
	return val
...
func LPush(key string, date ...interface{}) int64 {
	result, err := RedisClient.LPush(ctx, key, date).Result()
	if err != nil {
		fmt.Println(err)
	}
	return result
...
func RPush(key string, date ...interface{}) int64 {
	result, err := RedisClient.RPush(ctx, key, date).Result()
	if err != nil {
		fmt.Println(err)
	}
	return result
...
func LRange(key string, start, stop int64) []string {
	vales, err := RedisClient.LRange(ctx, key, start, stop).Result()
	if err != nil {
		fmt.Println(err)
	}
	return vales
...
func LRem(key string, count int64, data interface{}) bool {
	_, err := RedisClient.LRem(ctx, key, count, data).Result()
	if err != nil {
		fmt.Println(err)
	}
	return true
...
func LInsert(key string, pivot int64, data interface{}) bool {
	err := RedisClient.LInsert(ctx, key, "after", pivot, data).Err()
	if err != nil {
		fmt.Println(err)
		return false
	}
...
func SAdd(key string, data ...interface{}) bool {
	err := RedisClient.SAdd(ctx, key, data).Err()
	if err != nil {
		fmt.Println(err)
		return false
	}
...
func SIsMember(key string, data interface{}) bool {
	ok, err := RedisClient.SIsMember(ctx, key, data).Result()
	if err != nil {
		fmt.Println(err)
	}
	return ok
...
func SMembers(key string) []string {
	es, err := RedisClient.SMembers(ctx, key).Result()
	if err != nil {
		fmt.Println(err)
	}
	return es
...
func SRem(key string, data ...interface{}) bool {
	_, err := RedisClient.SRem(ctx, key, data).Result()
	if err != nil {
		fmt.Println(err)
		return false
	}
...
func SPopN(key string, count int64) []string {
	vales, err := RedisClient.SPopN(ctx, key, count).Result()
	if err != nil {
		fmt.Println(err)
	}
	return vales
...
func HMGet(key string, fields ...string) []interface{} {
	vales, err := RedisClient.HMGet(ctx, key, fields...).Result()
	if err != nil {
		panic(err)
	}
	return vales
...
func HKeys(key string) []string {
	fields, err := RedisClient.HKeys(ctx, key).Result()
	if err != nil {
		fmt.Println(err)
	}
	return fields
...
func HLen(key string) int64 {
	size, err := RedisClient.HLen(ctx, key).Result()
	if err != nil {
		fmt.Println(err)
	}
	return size
...
func HMSet(key string, data map[string]interface{}) bool {
	result, err := RedisClient.HMSet(ctx, key, data).Result()
	if err != nil {
		fmt.Println(err)
		return false
	}
...
func HSetNX(key, field string, value interface{}) bool {
	result, err := RedisClient.HSetNX(ctx, key, field, value).Result()
	if err != nil {
		fmt.Println(err)
		return false
	}
...
func HExists(key, field string) bool {
	result, err := RedisClient.HExists(ctx, key, field).Result()
	if err != nil {
		fmt.Println(err)
		return false
	}
